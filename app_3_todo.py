...
def safe_sentiment(text: str) -> str:
    try:
        # Thử gọi sentiment từ underthesea
        result = sentiment(text)

        # xử lý nếu trả về list / tuple
        ...
        # Nếu lỗi, trả về kết quả rule-based thay vì error
...
"""
        ...
        if "TÍCH CỰC" in label:
            st.success(f" P ({pos_total}) > N ({neg_total})  →  **{label}**  (Score = {score:+.2f})")
        elif "TIÊU CỰC" in label:
            st.error(f" N ({neg_total}) > P ({pos_total})  →  **{label}**  (Score = {score:+.2f})")
        else:
            st.warning(f" P ({pos_total}) = N ({neg_total})  →  **{label}**  (Score = {score:+.2f})")
        ...
        if "positive" in senti_label.lower():
            st.success(f"Nhãn underthesea: `{senti_label}`")
        elif "negative" in senti_label.lower():
